...
initialState = { counter = 0, bookModel = Book.init, display = CounterSect, avorionModel = Nothing}

main =
    Browser.element { init = init, view = view, update = update, subscriptions = subscriptions }

init : () -> (Model, Cmd Msg)
init _ = ( initialState,Cmd.map AvorionMsg Avorion.initCmd )
...
update msg model =
  case msg of
    ...
    BookMsg bMsg ->
      let (bModel, bCmd) = Book.update bMsg model.bookModel
      in
      ({model | bookModel = bModel}, Cmd.map BookMsg bCmd)
    AvorionMsg aMsg ->
      let (aModel, aCmd) = Avorion.update aMsg model.avorionModel
      in
  ...
  let
    subv =
      (case model.display of
        BookSect ->
          let v = Book.view model.bookModel
          in
            Html.map BookMsg v
        CounterSect -> viewCounter model
        AvorSect ->
          let v = Avorion.view model.avorionModel
          in
